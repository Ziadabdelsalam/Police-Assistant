...
class Car_Plate_Detection:
    def __init__(self):
        #Model Configuration For Car Plates
    ...
    def Detect_Plate(self, frame, Image_Counter):
        ...
        if rec > 0 and type(plateImg) == type(x) and plateImg.all() is not None:
            cv2.imwrite("Plates From Model/" + str(Image_Counter) + ".png", plateImg.astype(np.uint8))
            return True
